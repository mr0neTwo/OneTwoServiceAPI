...
def add_orders(
    ...
    try:
        ...
        if r_filter:
            ...
            query = query.order_by(
                getattr(Orders, r_filter.get('field_sort'), 'id') if r_filter.get('sort') == 'asc' else desc(getattr(Orders, r_filter.get('field_sort'), 'id')))
...
def get_orders(
    ...
    try:
        ...
        print(f'Время формирования запроса: {time_rec:.4} сек')
        print(f'Время определения количества: {time_count:.4} сек')
        print(f'Время формирования данных: {time.time() - start_time:.4} сек')
